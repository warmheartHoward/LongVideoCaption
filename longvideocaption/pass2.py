...
3. 【保留名单 preserve_temp_names】**逐 temp_name 决定是否保留原样**。⚠️ 强默认 `[]`（空列表）= **全部替换**。本环节的核心目的就是把"同一角色在不同 chunk 的别名"统一到权威名，**消除别名漂移是 95%+ 情况的目标**。请只在极少数有充分理由的情况下把临时名列入 preserve；**有疑问就替换**。

   ✅ **必须替换**（不要列入 preserve）的常见模式 —— 这正是本环节要解决的问题：
   * 临时名是纯描述性代号（基于颜色 / 物种 / 服饰 / 体型，如某种"颜色+形态"代号、"某衣某色男子"、"男人1"），仅因 Phase A 当时还不知道角色真名才使用。一旦聚类与有专名的权威角色合并，必须替换为权威名。
   * 临时名比权威名更模糊（权威名是专有称谓 / 真名 / 有辨识度的绰号；临时名是泛化描述）。
   * 临时名与权威名仅风格不同但指代同一身份且同一状态（如 [红衣女] 与 [赤衣女子]）。

   🚫 **极少数才列入 preserve** —— 临时名必须显式承载权威名无法表达的剧情信息：
   * 形态切换：该 sighting 处于与权威名明显不同的存在状态，临时名捕捉了这种状态（典型：活体 ↔ 骸骨/鬼魂/重伤/变形）。
   * 关键叙事节点的伪装：如"蒙面人"在揭面前后分别出现，揭面瞬间是剧情转折点，统一抹平会丢失这一节点。
   * 化身 / 年龄跨度极大且具体形象差异显著：权威名抹平后读者将无法识别该状态。

   【硬性自检】决定列入 preserve 之前，请反问自己：把"权威名"替进 caption 替换"该临时名"后，**读者是否会因此误判角色当时的状态/身份？**
   - 只是用词不同、指代相同 → 必须替换。
   - 读者会真的搞错状态/形态 → 才保留。
