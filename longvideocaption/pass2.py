...
3. 【保留名单 preserve_temp_names】**逐 temp_name 决定是否保留原样**。强默认 `[]`（空列表）= **全部替换**。本环节的核心目的就是把"同一角色在不同 chunk 的别名"统一到权威名，**消除别名漂移是 95%+ 情况的目标**。请只在极少数有充分理由的情况下把临时名列入 preserve；**有疑问就替换**。
